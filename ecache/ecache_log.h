...
#define ECACHE_ERROR(...)                                                                  \
  do {                                                                                     \
    if (ecache::Logger::GetDidagleLogger().ShouldLog(spdlog::level::err)) {                \
      std::string s = fmt::format(__VA_ARGS__);                                            \
      ecache::Logger::GetDidagleLogger().Log(                                              \
...
#define ECACHE_INFO(...)                                                                    \
  do {                                                                                      \
    if (ecache::Logger::GetDidagleLogger().ShouldLog(spdlog::level::info)) {                \
      std::string s = fmt::format(__VA_ARGS__);                                             \
      ecache::Logger::GetDidagleLogger().Log(                                               \
...
#define ECACHE_DEBUG(...)                                                                    \
  do {                                                                                       \
    if (ecache::Logger::GetDidagleLogger().ShouldLog(spdlog::level::debug)) {                \
      std::string s = fmt::format(__VA_ARGS__);                                              \
      ecache::Logger::GetDidagleLogger().Log(                                                \
